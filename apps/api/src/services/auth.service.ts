  ...
  async authenticate(
    ...
    try {
      ...
      const { user: dbUser, isNewUser } =
        await this.findOrCreateUser(googlePayload);
  ...
  private async analyzeSecurityFlags(
    ...
    if (context.fingerprint) {
      ...
      const hasSeenFingerprint = recentLogins.some(log => {
        try {
          const details: { fingerprint?: string } = log.details
            ? (JSON.parse(log.details) as { fingerprint?: string })
            : {};
  ...
  async checkRateLimit(ip: string, email?: string): Promise<RateLimitResult> {
    ...
    if (email) {
      const emailAttempts = await this.db
        ...
        .from(auditLogs)
        .where(
          and(eq(auditLogs.email, email), eq(auditLogs.action, 'login_failure'))
        )
        .limit(10);
